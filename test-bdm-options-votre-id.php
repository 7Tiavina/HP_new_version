<?php
/**
 * Test endpoint commande/options - Avec votre ID plateforme
 */
...
$plateformeId = '64f00ace-31b6-45b0-bcb2-b562b1ac08d9'; // Votre ID

echo "=== TEST COMMANDE OPTIONS (Votre ID plateforme: $plateformeId) ===\n\n";
...
// Étape 1: Récupérer les produits pour CETTE plateforme
...
echo "   Réponse produits:\n" . json_encode($productsResult, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) . "\n\n";

// Vérifier statut
$success = isset($productsResult['statut']) && $productsResult['statut'] == 1 
           || isset($productsResult['isSucceed']) && $productsResult['isSucceed'];

if ($success && isset($productsResult['content'])) {
    ...
    if ($firstProduct) {
        ...
        // Étape 2: Tester commande/options
        ...
        // Analyse
        echo "3. Analyse...\n";
        $optionsSuccess = isset($data['statut']) && $data['statut'] == 1 
                       || isset($data['isSucceed']) && $data['isSucceed'];
