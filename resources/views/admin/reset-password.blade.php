...
<head>
    ...
    <title>Réinitialiser le mot de passe administrateur - HelloPassenger</title>
...
<body class="bg-gray-100">
    <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-lg">
            ...
            <form class="mt-8 space-y-6" method="POST" action="{{ route('admin.reset-password') }}">
                @csrf
                <input type="hidden" name="token" value="{{ request()->query('token') }}">
                
                <div>
                    <label for="email" class="block text-sm font-medium text-gray-700 mb-2">Email du compte administrateur</label>
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-md p-4">
                    <p class="text-sm text-yellow-800">
                        <strong>Note :</strong> Un nouveau mot de passe sécurisé sera généré automatiquement et envoyé par email à mahnish@blablabla-agency.com
                    </p>
                </div>
